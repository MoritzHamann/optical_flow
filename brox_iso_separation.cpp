  ...
  for (int l = maxlevel; l >= 0; l--){
    ...
    for (int i = 0; i < maxiter; i++){
      for (int j = 0; j < iter_flow_before_phi; j++){
        ...
        Brox_step_iso_smooth(t, flowfield_p, flowfield_m, partial_p, partial_m, data_p, data_m, smooth_p, smooth_m, phi, parameters, h);
      }
  ...
  for (int i = 1; i < p.rows-1; i++){
    for (int j = 1; j < p.cols-1; j++){
      ...
      if (phi(i,j)*sign > 0){
        ...
        xm = (j > 1) * (L1dot(smooth(i,j-1)) + L1dot(smooth(i,j)))/2.0 * (H(phi(i,j-1)*sign) + H(phi(i,j)*sign))/2.0 * alpha/(h*h);
        xp = (j < p.cols - 2) * (L1dot(smooth(i,j+1)) + L1dot(smooth(i,j)))/2.0 * (H(phi(i,j+1)*sign) + H(phi(i,j)*sign))/2.0 * alpha/(h*h);
        ym = (i > 1) * (L1dot(smooth(i-1,j)) + L1dot(smooth(i,j)))/2.0 * (H(phi(i-1,j)*sign) + H(phi(i,j)*sign))/2.0 * alpha/(h*h);
        yp = (i < p.rows - 2) * (L1dot(smooth(i+1,j)) + L1dot(smooth(i,j)))/2.0 * (H(phi(i+1,j)*sign) + H(phi(i,j)*sign))/2.0 * alpha/(h*h);
        sum = xm + xp + ym + yp;
  ...
  for (int i = 1; i < phi.rows-1; i++){
    for (int j = 1; j < phi.cols-1; j++){
      ...
      phi(i,j) = (1.0/c)*(phi(i,j) + m*(c1*phi(i,j+1)+c2*phi(i,j-1)+c3*phi(i+1,j)+c4*phi(i-1,j))
                          ...
                          -deltat*alpha*Hdot(phi(i,j))*(L1(smooth_p(i,j)) - L1(smooth_m(i,j))));
    }
  }
