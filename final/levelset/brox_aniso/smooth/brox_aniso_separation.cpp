...
void updatePhi(const cv::Mat_<cv::Vec2d> &fp,
  ...
  for (int i = 1; i < phi.rows-1; i++){
    for (int j = 1; j < phi.cols-1; j++){
      ...
      sp = smooth_p(i,j)[0] * fp(i,j)[0] + smooth_p(i,j)[1] * fp(i,j)[1] + smooth_p(i,j)[1] * fp(i,j)[0] + smooth_p(i,j)[2] * fp(i,j)[1];
      sm = smooth_m(i,j)[0] * fm(i,j)[0] + smooth_m(i,j)[1] * fm(i,j)[1] + smooth_m(i,j)[1] * fm(i,j)[0] + smooth_m(i,j)[2] * fm(i,j)[1];
