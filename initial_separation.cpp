...
void segementFlowfield(const cv::Mat_<cv::Vec2d> &f, cv::Mat_<double> &phi, const std::unordered_map<std::string, parameter> &parameters){
  ...
  std::cout << "valid blocks computed" << std::endl;
  ...
  for (int i = 0; i < num_y; i = i+3) {
    for (int j = 0; j < num_x; j = j+3) {
  ...
  std::cout << merged_blocks_best << std::endl;
  std::cout << best_affine << std::endl;

  double error = 0;

  for (int i = 0; i < f.rows; i++) {
    for (int j = 0; j < f.cols; j++) {
      error  = std::pow(best_affine[0] * i + best_affine[2] * j + best_affine[4] - f(i,j)[0], 2);
      error += std::pow(best_affine[1] * i + best_affine[3] * j + best_affine[5] - f(i,j)[1], 2);
